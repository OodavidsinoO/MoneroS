...
if [ "$USE_PREDEFINED_BUSYBOX_CONFIG" = "true" -a ! -f $SRC_DIR/moneros_config/busybox.config ] ; then
  echo "Config file $SRC_DIR/moneros_config/busybox.config does not exist."
...
if [ "$USE_PREDEFINED_BUSYBOX_CONFIG" = "true" ] ; then
  ...
  echo "Using config file $SRC_DIR/moneros_config/busybox.config"
  cp -f $SRC_DIR/moneros_config/busybox.config .config
else
