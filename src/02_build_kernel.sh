...
if [ "$USE_PREDEFINED_KERNEL_CONFIG" = "true" -a ! -f $SRC_DIR/moneros_config/kernel.config ] ; then
  echo "Config file '$SRC_DIR/moneros_config/kernel.config' does not exist."
...
if [ "$USE_PREDEFINED_KERNEL_CONFIG" = "true" ] ; then
  ...
  echo "Using config file '$SRC_DIR/moneros_config/kernel.config'."
  cp -f $SRC_DIR/moneros_config/kernel.config .config
else
