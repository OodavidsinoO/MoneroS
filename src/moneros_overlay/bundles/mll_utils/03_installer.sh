...
# This script installs MoneroS Linux Live on Ext2 partition.
...
if [ "\$PRINT_HELP" = "true" ] ; then
  ...
  This is the MoneroS Linux Live installer. Requires root permissions.

  Usage: mll-install DEVICE

  DEVICE    The device where MoneroS Linux Live will be installed. Specify only
  ...
  The above example installs MoneroS Linux Live  on '/dev/sdb'.
...
cat << DEOF
  ...
  MoneroS Linux Live will be installed on device '/dev/\$1'. The device will be
...
echo "MoneroS Linux Live installer has been generated."
