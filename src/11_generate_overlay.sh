...
if [ "$OVERLAY_LOCATION" = "iso" ] && \
  ...
  # All files and folders located in the folder 'moneros_overlay' will be merged
  ...
  cp -r $SRC_DIR/moneros_overlay/rootfs/* \
    $ISOIMAGE_OVERLAY/sparse/rootfs
  ...
  # the folder 'moneros_overlay' will be merged with the root folder on boot.
